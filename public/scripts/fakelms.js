...
FakeLMS.attachLMSAPIToWindow = function() {
	...
	window.API = new FakeLMSAPI();
}

FakeLMS.clearData = function() {
	['exit','core.session_time','suspend_data','core.lesson_status','success_status','completion_status','interactions'].forEach(window.localStorage.removeItem.bind(window.localStorage));
...
FakeLMSAPI.prototype.LMSInitialize = function(wtf) {
...
*/
FakeLMSAPI.prototype.LMSGetValue = function(path) {
	...
	if ( -1 != ["exit","session_time"].indexOf(parts[1]) || -1 != ["session_time"].indexOf(parts[2])) {
...
FakeLMSAPI.prototype.LMSSetValue = function(path, value) {
	...
	switch (parts[1]) {
		case 'exit':
			if (-1==["timeout", "logout", "normal", ""].indexOf(value)) {
		...
		case 'core':
					switch(parts[2]){
						case 'session_time':
							console.log(`Value is ${value}`);
							window.localStorage.setItem('session_time', value);
							window.dispatchEvent(event);
							break;
						case 'lesson_status':
							console.log(`Value is ${value}`);
							window.localStorage.setItem('lesson_status', value);
							window.dispatchEvent(event);
							break;
						default:
							break;
...
*/
FakeLMSAPI.prototype.LMSCommit = function(wtf) {
	if ("string" != typeof wtf || wtf.length) {
		return this._fail(FakeLMS.ERRCODES.GENERAL_ARGUMENT_ERROR);
	}
	return this._ok();
...
FakeLMSAPI.prototype.LMSGetLastError = function() {
	return this.lastErrcode;
}

FakeLMSAPI.prototype.LMSGetDiagnostic = function(p) {
	return p;
}

FakeLMSAPI.prototype.LMSGetErrorString = function(errCode) {
